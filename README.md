...
Current capabilities include tenant onboarding, API-key/JWT authentication, audit log reads, tenant rate limiting, notification templates with variables, notification intake, notification status listing, tenant analytics, idempotent request handling, BullMQ enqueueing, worker-based delivery processing, mock channel providers, retry/backoff handling, dead-letter status, and delivery attempt history.
